...
export default function EditLostItemPage() {
    ...
    fetch(`/api/lost/${id}`)
      ...
      .then((data) => {
        setForm({
          ...
          lostAt: data.lostAt ? new Date(data.lostAt).toISOString().slice(0, 16) : "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    else { router.push("/dashboard/lost"); router.refresh(); }
  }

  async function handleDelete() {
    setLoading(true);
    const res = await fetch(`/api/lost/${id}`, { method: "DELETE" });
    if (res.ok) { router.push("/dashboard/lost"); router.refresh(); }
  ...
  return (
    <div style={{ maxWidth: "680px" }}>
      <div className="page-header">
        <h1 className="page-title">تعديل بلاغ المفقود</h1>
      ...
      {deleteConfirm && (
        <div className="alert alert-error" style={{ marginBottom: "var(--space-6)" }}>
          <p style={{ marginBottom: "var(--space-4)" }}>هل أنت متأكد من حذف هذا البلاغ؟ لا يمكن التراجع.</p>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "var(--space-6)" }}>
        ...
        <div className="field">
          <label className="label" htmlFor="lostAt">تاريخ الفقدان</label>
          <input id="lostAt" name="lostAt" type="datetime-local" className="input" value={form.lostAt} onChange={handleChange} dir="ltr" />
        </div>
